...
![alt text](Screen_5.png)

Значение альфа = 1000 является вполне ожидаемым и даже логичным для данного набора данных
Это показывает, что регуляризация работает именно так, как задумано.
Обучающая выборка состоит всего из 1595 наблюдений.
У нас гораздо больше признаков, чем наблюдений.
Модель легко может выучить шум и идеально подстроиться под обучающие данные,
что мы и видим по R² = 1.000 на обучении для полинома степени 2.
Это яркий признак жесткого переобучения.